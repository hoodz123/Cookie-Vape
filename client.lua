local vaping = false
RegisterNetEvent("cc-vape:client:use", function()
	...
	if (DoesEntityExist(ped) and not IsEntityDead(ped)) and not vaping then
		...
		Wait(950)
		TriggerServerEvent("cc-vape:server:effects", PedToNet(ped), coords)
		Wait(950)
		DeleteObject(vape)
...
RegisterNetEvent("cc-vape:client:effects", function(ped, coords)
